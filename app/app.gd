...
var _play_animations: bool:
	get: return _play_animations

	set(value):
		_play_animations = value

		for sample: Node3D in _samples.get_children():
			if sample.has_method("animation_toggled"):
				sample.animation_toggled(_play_animations)
...
@onready
var _animation_button: Button = %Animation

@onready
var _animation_button_base_text := _animation_button.text
...
func _process(_delta: float) -> void:
	...
	_animation_button.button_pressed = _play_animations
	_animation_button.text = (
		_animation_button_base_text % _format_bool(_play_animations, "PLAYING", "PAUSED"))
